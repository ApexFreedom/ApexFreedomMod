...
public final class DonatorWorld extends CustomWorld
{
    ...
    {
        if (guest == supervisor || plugin.dl.isDonator(guest))
        {
            return false;
        }

        if (plugin.dl.isDonator(supervisor))
        {
    ...
    public boolean validateMovement(PlayerMoveEvent event)
    {
        ...
        if (lastTP == null || lastTP + TP_COOLDOWN_TIME <= currentTimeMillis)
        {
            ...
            FLog.info(player.getName() + " attempted to access the DonatorWorld.");
    ...
    public boolean canAccessWorld(final Player player)
    {
        ...
        if (cached == null)
        {
            boolean canAccess = plugin.dl.isDonator(player);
            if (!canAccess)
            {
