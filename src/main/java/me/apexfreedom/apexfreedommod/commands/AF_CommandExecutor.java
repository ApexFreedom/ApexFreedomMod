...
public class AF_CommandExecutor<C extends AeroCommandBase<?>> extends AbstractCommandExecutor<C>
{

    private final TotalFreedomMod plugin;

    public AF_CommandExecutor(TotalFreedomMod plugin, AeroCommandHandler<?> handler, String name, C command)
    {
        super(handler, name, command);
        this.plugin = plugin;
    }

    protected AF_Command getCommand()
    {
        return commandBase instanceof AF_Command ? (AF_Command) commandBase : null;
    }

    @Override
    public void setupCommand(PluginCommand pluginCommand)
    {
        final AF_Command command = getCommand();
        if (command == null)
        {
            return;
        }
    ...
    public static class FreedomExecutorFactory implements AeroCommandExecutorFactory
    {
        ...
        {
            return new AF_CommandExecutor<>(plugin, handler, name, command);
        }
