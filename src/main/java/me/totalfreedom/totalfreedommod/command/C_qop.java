...
public class C_qop extends FreedomCommand
{
    ...
    public boolean run(CommandSender sender, Player playerSender, Command cmd, String commandLabel, String[] args, boolean senderIsConsole)
    {
        ...
        if (!matchedPlayerNames.isEmpty())
        {
            if (!silent)
            {
                FUtil.adminAction(ChatColor.BLUE + sender.getName(), "Opping " + StringUtils.join(matchedPlayerNames, ", "), false);
            }
        }
        else
        {
            msg(FMsg.F + ChatColor.RED + "No targets matched.");
        }
