...
public class ServerPing extends FreedomService
{
    ...
    public void onServerPing(ServerListPingEvent event)
    {
        ...
        {
            event.setMotd(FMsg.F + ChatColor.RED + "You are banned.");
            return;
        }
        
        if (ConfigEntry.ADMIN_ONLY_MODE.getBoolean())
        {
            event.setMotd(FMsg.F + ChatColor.RED + "Server is closed.");
            return;
        }

        if (Bukkit.hasWhitelist())
        {
            event.setMotd(FMsg.F + ChatColor.RED + "Whitelist enabled.");
            return;
        }

        if (Bukkit.getOnlinePlayers().size() >= Bukkit.getMaxPlayers())
        {
            event.setMotd(FMsg.F + ChatColor.RED + "The server is full");
            return;
        }

        String baseMotd = ConfigEntry.SERVER_MOTD.getString().replace("*mcv*", plugin.si.getVersion());
