...
public class ChatManager extends FreedomService
{
    ...
    public void adminChat(CommandSender sender, String message)
    {
        ...
        for (Player player : server.getOnlinePlayers())
        {
            ...
            {
                player.sendMessage("[" + ChatColor.AQUA + "AdminChat" + ChatColor.WHITE + "] " + ChatColor.DARK_RED + name + ": " + ChatColor.GOLD + message);
            }
        }
    }
    ...
    public void reportAction(Player reporter, Player reported, String report)
    {
        for (Player player : server.getOnlinePlayers())
        {
            if (plugin.al.isAdmin(player))
            {
                playerMsg(player, ChatColor.DARK_GRAY + "[" + ChatColor.BLUE + "ApexFreedomReports" +
                        ChatColor.DARK_GRAY + "] " + ChatColor.GREEN + reporter.getName() +
                        ChatColor.RED + " has reported " + reported.getName() + " for " + report);
            }
        }
    }
